...
while IFS= read -r USER; do
    ...
    if [[ $(echo "$ACCESS_KEYS" | jq '.AccessKeyMetadata | length') -gt 0 ]]; then
	    ...
	    echo "HIIIIIIIII The oldest key is $OLDEST_ACTIVE_KEY_ID and was last used on $LAST_USED_DATE_HUMAN" >> $logfile 
	    ...
	    if [[ -n "$OLDEST_ACTIVE_KEY_ID" ]]; then
		...
		CURRENT_EPOCH=$(date +%s)
		if (( (CURRENT_EPOCH - ACTIVE_CREATION_EPOCH) > (480) )); then
